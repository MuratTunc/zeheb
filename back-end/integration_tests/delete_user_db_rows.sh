...
if [ -z "$CONTAINER_ID" ]; then
  echo "Error: No running container found with name '$USER_POSTGRES_DB_CONTAINER_NAME'."
  exit 1
fi
...
docker exec -i "$CONTAINER_ID" psql -U "$USER_POSTGRES_DB_USER" -d "$USER_POSTGRES_DB_NAME" -c "DELETE FROM users;"
